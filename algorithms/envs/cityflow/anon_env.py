...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict,path_to_log=None):
...
class AnonEnv:
    ...
    def __init__(self, path_to_log, path_to_work_directory, dic_traffic_env_conf):
        self.path_to_log = path_to_log
        self.path_to_work_directory = "./algorithms/envs/cityflow/data/template_lsr/6_6"
    ...
    def step(self, action):
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            done = [self.get_done()] * len(self.list_intersection)

        #print("Step time: ", time.time() - step_start_time)
        return np.array(next_state), np.array(reward), np.array(done), np.array(average_reward_action_list)
